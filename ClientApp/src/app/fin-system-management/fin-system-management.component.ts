...
export class FinSystemManagementComponent implements OnInit {
  ...
  onNew() {
    this.router.navigate([`partner-area/fin-system/0/0`]);
  }

  edit(obj: FinSystem) {
    this.router.navigate([`partner-area/fin-system/${obj.id}/1`]);
  }
  ...
  onActive(item) {
    this.finSystemService.active(item).subscribe(result => {
      this.onSubmit();
    });
  }
